//------------------------------------------------------------------------------
// Author      : Brian Liau (brian.liau@tufts.edu)
// File        : IDEX_register.sv
// Design Unit : IDEX_register
// Description : Instruction Decode (ID) to Execution (EX) pipeline stage 
//               register. Latches ID stage evaluation results and control 
//               signal, holding state during bus/pipeline stalls and flushing
//               instructions during pipeline flushes.
// Dependencies: rvDefs.sv
// Target      : Synthesis
//------------------------------------------------------------------------------
module IDEX_register (
    input  logic                    clk,
    input  logic                    resetN,
    input  logic                    pipelineStall,
    input  logic                    flush,
    input  logic                    IDEX_delay,
    input  rvDefs::xreg_t           read1Data_ID,
    input  rvDefs::xreg_t           read2Data_ID,
    input  rvDefs::xreg_addr_t      rs1_ID,
    input  rvDefs::xreg_addr_t      rs2_ID,
    input  rvDefs::xreg_addr_t      rd_ID,
    input  rvDefs::mem_addr_t       instructionAddress_ID,
    input  rvDefs::word_t           immediate_ID,
    input  rvDefs::xalu_op_t        xaluOp_ID,
    input  logic                    xaluArithmeticFlag_ID,
    input  rvDefs::memory_op_size_t memoryOpSize_ID,
    input  logic                    unsignedLoad_ID,
    input  logic                    storeLoad_ID,
    input  logic                    writeEnable_ID,
    input  rvDefs::write_src_t      writeSource_ID,
    input  logic                    pcXaluPrimary_ID,
    input  logic                    immediateXaluSecondary_ID,
    input  logic                    zeroXaluPrimary_ID,
    input  rvDefs::branch_op_t      branchOp_ID,
    input  logic                    branchNegate_ID,
    input  rvDefs::instruction_t    instruction_ID,         
    input  logic                    jump_ID,
    input  logic                    JALR_ID,
    input  logic                    branchPrediction_ID,
    output logic                    branchPrediction_EX,
    output logic                    JALR_EX,
    output rvDefs::branch_op_t      branchOp_EX,
    output logic                    branchNegate_EX,
    output logic                    jump_EX,
    output logic                    zeroXaluPrimary_EX,
    output logic                    immediateXaluSecondary_EX,
    output rvDefs::write_src_t      writeSource_EX,
    output logic                    writeEnable_EX,
    output rvDefs::xreg_t           read1Data_EX,
    output rvDefs::xreg_t           read2Data_EX,
    output rvDefs::xreg_addr_t      rs1_EX,
    output rvDefs::xreg_addr_t      rs2_EX,
    output rvDefs::xreg_addr_t      rd_EX,
    output rvDefs::mem_addr_t       instructionAddress_EX,
    output rvDefs::word_t           immediate_EX,
    output rvDefs::xalu_op_t        xaluOp_EX,
    output logic                    xaluArithmeticFlag_EX,
    output rvDefs::memory_op_size_t memoryOpSize_EX,
    output logic                    unsignedLoad_EX,
    output logic                    storeLoad_EX,
    output logic                    pcXaluPrimary_EX,
    output rvDefs::instruction_t    instruction_EX
);
    // Latch ID stage signals into EX stage registers on active clock edge
    always_ff @(posedge clk or negedge resetN) begin
        if (!resetN) begin
            read1Data_EX <= 32'b0;
            read2Data_EX <= 32'b0;
            rs1_EX <= 5'b0;
            rs2_EX <= 5'b0;
            rd_EX <= 5'b0;
            immediate_EX <= 32'b0;
            instructionAddress_EX <= 32'b0;
            xaluOp_EX <= rvDefs::XALU_OP_SUM;
            xaluArithmeticFlag_EX <= 1'b0;
            memoryOpSize_EX <= rvDefs::MEMORY_OP_SIZE_NONE;
            unsignedLoad_EX <= 1'b0;
            storeLoad_EX <= 1'b0;
            writeEnable_EX <= 1'b0;
            writeSource_EX <= rvDefs::WRITE_SRC_NONE;
            immediateXaluSecondary_EX <= 1'b0;
            zeroXaluPrimary_EX <= 1'b0;
            branchOp_EX <= rvDefs::BRANCH_OP_NONE;
            instruction_EX <= rvDefs::NOP;
            pcXaluPrimary_EX <= 1'b0;
            branchNegate_EX <= 1'b0;
            branchPrediction_EX <= 1'b0;
            jump_EX <= 1'b0;
            JALR_EX <= 1'b0;
        end else begin
            // Flush pipelined signals during pipeline flush and hazard detected
            if (flush || IDEX_delay) begin
                rd_EX           <= 5'b0;
                writeEnable_EX  <= 1'b0;
                writeSource_EX  <= rvDefs::WRITE_SRC_NONE;
                memoryOpSize_EX <= rvDefs::MEMORY_OP_SIZE_NONE;
                storeLoad_EX    <= 1'b0;
                unsignedLoad_EX <= 1'b0;
                xaluOp_EX       <= rvDefs::XALU_OP_SUM;
                xaluArithmeticFlag_EX <= 1'b0;
                immediateXaluSecondary_EX <= 1'b0;
                pcXaluPrimary_EX <= 1'b0;
                read1Data_EX     <= 32'b0;
                read2Data_EX     <= 32'b0;
                rs1_EX           <= 5'b0;
                rs2_EX          <= 5'b0;
                immediate_EX    <= 32'b0;
                instructionAddress_EX <= 32'b0;
                zeroXaluPrimary_EX <= 1'b0;
                branchOp_EX <= rvDefs::BRANCH_OP_NONE;
                instruction_EX <= rvDefs::NOP;
                branchNegate_EX <= 1'b0;
                branchPrediction_EX <= 1'b0;
                jump_EX <= 1'b0;
                JALR_EX <= 1'b0;
            end
            // Hold pipeline signals when pipeline is stalled
            else if (pipelineStall) begin
                read1Data_EX <= read1Data_EX;
                read2Data_EX <= read2Data_EX;
                rs1_EX <= rs1_EX;
                rs2_EX <= rs2_EX;
                rd_EX <= rd_EX;
                immediate_EX <= immediate_EX;
                instructionAddress_EX <= instructionAddress_EX;
                xaluOp_EX <= xaluOp_EX;
                xaluArithmeticFlag_EX <= xaluArithmeticFlag_EX;
                memoryOpSize_EX <= memoryOpSize_EX;
                unsignedLoad_EX <= unsignedLoad_EX;
                storeLoad_EX <= storeLoad_EX;
                writeEnable_EX <= writeEnable_EX;
                writeSource_EX <= writeSource_EX;
                immediateXaluSecondary_EX <= immediateXaluSecondary_EX;
                zeroXaluPrimary_EX <= zeroXaluPrimary_EX;
                branchOp_EX <= branchOp_EX;
                instruction_EX <= instruction_EX;
                pcXaluPrimary_EX <= pcXaluPrimary_EX;
                branchNegate_EX <= branchNegate_EX;
                branchPrediction_EX <= branchPrediction_EX;
                jump_EX <= jump_EX;
                JALR_EX <= JALR_EX;
            end
            // Advance pipelined signals jf no delay, flushes, or stalls
            else begin
                read1Data_EX <= read1Data_ID;
                read2Data_EX <= read2Data_ID;
                rs1_EX <= rs1_ID;
                rs2_EX <= rs2_ID;
                rd_EX <= rd_ID;
                immediate_EX <= immediate_ID;
                instructionAddress_EX <= instructionAddress_ID;
                xaluOp_EX <= xaluOp_ID;
                xaluArithmeticFlag_EX <= xaluArithmeticFlag_ID;
                memoryOpSize_EX <= memoryOpSize_ID;
                unsignedLoad_EX <= unsignedLoad_ID;
                storeLoad_EX <= storeLoad_ID;
                writeEnable_EX <= writeEnable_ID;
                writeSource_EX <= writeSource_ID;
                immediateXaluSecondary_EX <= immediateXaluSecondary_ID;
                zeroXaluPrimary_EX <= zeroXaluPrimary_ID;
                branchOp_EX <= branchOp_ID;
                instruction_EX <= instruction_ID;
                pcXaluPrimary_EX <= pcXaluPrimary_ID;
                branchNegate_EX <= branchNegate_ID;
                branchPrediction_EX <= branchPrediction_ID;
                jump_EX <= jump_ID;
                JALR_EX <= JALR_ID;
            end 
        end
    end

endmodule
